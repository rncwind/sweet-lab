use data::routeguide::route_guide_server::{RouteGuide, RouteGuideServer};
use data::routeguide::{Feature, Point, Rectangle, RouteNote, RouteSummary};
...
impl RouteGuide for RouteGuideService {
    ...
    async fn record_route(
        ...
        unimplemented!("record_route is not implemented yet")
    }

    // A pinned type, is a type that is guaranteed to not move in memory.
    // A box is a heap allocation.
    // A dyn is a dynamic type, a type that is not known at compile time.
    // Specifically, it is a trait object, which is a stream, of items, where an item is a Result<RouteNote, Status>,
    // that also implements Send, and has a lifetime of 'static.
    ...
    async fn route_chat(
        ...
        unimplemented!("route_chat is not implemented yet")
    }
}
